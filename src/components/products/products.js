...
export default function Products({
  ...
  const [savedAnimating, setSavedAnimating] = useState(null);
  ...
  return (
    <div className={`container ${className}`}>
      ...
      {products.length > 0 ? (
          ...
          {products.map((product, index) => (
            ...
            >
              <div className="product-card mb-3">
                ...
                <HeartAdd
                  onClick={() => {
                    if (isInSaved(product.id)) {
                      removeSavedItem(product.id);
                    } else {
                      addSavedItem(product);
                    }
                    setSavedAnimating(product.id);
                    clearTimeout(savedTimeoutRef.current);
                    savedTimeoutRef.current = setTimeout(() => setSavedAnimating(null), 400);
                  }}
                  variant={isInSaved(product.id) ? "Bold" : "Bulk"}
                  size={28}
                  className={`product-saved pe-active${savedAnimating === product.id ? " animated" : ""}`}
                  style={{
        ...
        <div className="row">
          <div className="col-12 text-center no-products">
